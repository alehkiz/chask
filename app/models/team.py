...
class Team(BaseModel):
    ...
    users = db.relationship(
        ...
        backref=db.backref(
            "teams",
            lazy="dynamic",  # order_by="desc(team_administrators.c.administrator_at)"
        ),
        lazy="dynamic",
        # order_by="desc(team_administrators.c.administrator_at)",
        viewonly=True,
    ...
    tickets_stage_event = db.relationship(
        "TicketStageEvent", back_populates="team", viewonly=True
    )
    ...
    messages = db.relationship(
        "Message", backref="team", lazy="dynamic", order_by="asc(Message.create_at)"
    )
    ...
    def unreaded_messages(self, user):
        ...
        team_messages = (
            ...
            .filter(Message.team_id == self.id)
            .subquery()
        )
        read_msg = (
            db.session.query(db.func.count(Message.id).label("cnt"))
